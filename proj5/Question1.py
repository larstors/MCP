...
def Quant_Force(r1: np.ndarray, r2: np.ndarray, kappa: float, beta: float, alpha: float):
    ...
    force1 = - kappa * r1 / radius1 + beta * (r1 - r2) / (radius12 * (1 + alpha * radius12)**2)
    force2 = - kappa * r2 / radius2 - beta * (r1 - r2) / (radius12 * (1 + alpha * radius12)**2)
...
def FP_Greens(r: np.ndarray, y: np.ndarray, F: np.ndarray, dtau: float):
    ...
    return 1.0 / np.sqrt(2 * np.pi*dtau) * np.exp(- exponent / (2.0 * dtau))
...
def Metropolis_Monte_Carlo(M: int, N: int, n: int, s: float, kappa: float, beta: float, alpha: float, n_equil: int, dtau: float, method="uniform"):
    ...
    for i in range(N+1):
        ...
        if method=="uniform":
            ...
            for m in range(M):
                ...
                # add energy to sum
                energy[m] += Local_Energy(w1[3*m:3*m+3], w2[3*m:3*m+3], kappa, beta, alpha) 
                
                if i > n_equil:
                    total_local_energy[m] += Local_Energy(w1[3*m:3*m+3], w2[3*m:3*m+3], kappa, beta, alpha) 
        ...
        # Fokker Planck dynamics
            ...
            for m in range(M):
                ...
                F = Quant_Force(w1[3*m:3*m+3], w2[3*m:3*m+3], kappa, beta, alpha)
                ...
                # updating one of the two electrons of each walker
                if chose < 0.5:
                    trial_1 = w1[3*m:3*m+3] + 0.5*(np.random.rand(3)*2*s-s) + eta * np.sqrt(dtau) + dtau * F[0] / 2.0
                    trial_2 = w2[3*m:3*m+3]
                    trial_F = Quant_Force(trial_1[3*m:3*m+3], trial_2[3*m:3*m+3], kappa, beta, alpha)
                else:
                    trial_1 = w1[3*m:3*m+3]
                    trial_2 = w2[3*m:3*m+3] + 0.5*(np.random.rand(3)*2*s-s) + eta * np.sqrt(dtau) + dtau * F[1] / 2.0
                    trial_F = Quant_Force(trial_1[3*m:3*m+3], trial_2[3*m:3*m+3], kappa, beta, alpha)

                # probability after the updates
                rho_prime = Trial_Psi(trial_1, trial_2, kappa, beta, alpha)**2
                # accept probability
                P_acc = min(1, rho_prime / rho * FP_Greens(trial_1, w1[3*m:3*m+3], trial_F[0], dtau) / FP_Greens( w1[3*m:3*m+3], trial_1, F[0], dtau)\
                        * FP_Greens(trial_2, w2[3*m:3*m+3], trial_F[1], dtau) / FP_Greens( w2[3*m:3*m+3], trial_2, F[1], dtau))
    ...
    return index, mean_energy, variance, tot_E, tot_var_E
...
#         kappa=2.0, beta=0.5, alpha=0.15, n_equil=0, dtau=0, method="uniform") for ch in choice_s)
...
#         kappa=2.0, beta=0.5, alpha=al, n_equil=0, dtau=0, method="uniform") for al in alp)
...
result = Parallel(n_jobs=6)(delayed(Metropolis_Monte_Carlo)(M=300, N=N_it, n=1000, s=s_final, \
...
print("With N=%d we get the minima for E_L=%.3f and std(E_L)=%.3f at alpha = %.2f and %.2f, respectively" % (20000, E[min_index_E],varE[min_index_var_E], alp[min_index_E], alp[min_index_var_E]))
...
print("With N=%d we get the minima for E_L=%.3f and std(E_L)=%.3f at kappa = %.2f and %.2f, respectively" % (20000, E[min_index_E],varE[min_index_var_E], kap[min_index_E], kap[min_index_var_E]))
...
print("With N=%d we get the minima for E_L=%.3f and std(E_L)=%.3f at beta = %.2f and %.2f, respectively" % (20000, E[min_index_E],varE[min_index_var_E], bet[min_index_E], bet[min_index_var_E]))

res = Metropolis_Monte_Carlo(M=300, N=N_it, n=1000, s=s_final, kappa=1.85, alpha=0.18, beta=0.38, n_equil=n_eq, dtau=0)
